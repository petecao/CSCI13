...
public class GovermentWeather extends AbstractWeather {
    ...
    public void refreshData(String zipCode) {
        ...
        try {
            ...
            JsonArray iconLinks = jsonRoot.getAsJsonObject().get("data").getAsJsonObject().get("iconLink").getAsJsonArray();
            index = 0;
            for (JsonElement element : iconLinks) {
            ...
            notifyModifiedWeatherListeners();
        }
        catch (IOException e) {
            notifyErrorWeatherListeners();
        }
    }
